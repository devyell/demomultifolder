...
appController.controller('ctrlCambioPlanMovil', ['$scope', '$http', '$timeout', 'srvCambioPlanMovil', '$httpParamSerializer',
    function($scope, $http, $timeout, srvCambioPlanMovil, $httpParamSerializer) {
        ...
        var categoriaCorporativo = WPSCategoria.movil;
        $scope.mostrarServicios = false;
        $scope.showSinServicios = false;
        $scope.errorTotalRedirect = false;
        $scope.mostrarDataCambioPlan = false;
        var urlConsumer = '/wps/myportal/miclaro/consumer/solicitudes/cambiodeplan/movil';
        var codOperacionCambioPlan = WPSTablaOperaciones.solicitarCambioPlan;
        var pageIdeCambioPlan = WPSPageID.miclaro_corporativo_solicitudes_cambiodeplan_movil;
        ...
        function init() {
            srvCambioPlanMovil.obtenerDatosUsuario().then(function(response) {
                ...
                if (rpta == 0) {
                    ...
                    if (flagMovilSesion == 2 || flagMovilSesion == 3) {
                        $scope.mostrarDataCambioPlan = true;
                    } else if (flagMovilSesion == -1) {
                        $scope.errorTotalRedirect = true;
                        var mensajeAuditoria = 'obtenerServicios -' + 'flagMovil';
                        guardarAuditoria(flagMovilSesion, idTransaccion, mensajeAuditoria);
                    } else {
        ...
        $scope.solicitarNuevoServicio = function() {
            ...
            srvCambioPlanMovil.solicitarNuevoServicios(dataPlan).then(function(response) {
        ...
        function guardarAuditoria(rpta, idTransaccion,mensajeAuditoria) {
            ...
            srvCambioPlanMovil.guardarAuditoria(dataAuditoria).then(function(response) {
            }, function(error) {
